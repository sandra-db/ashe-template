...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    initCookieConsent();
    await insertHead("Weekly hours worked");
    insertHeader();
    insertNavButtons();
    insertFooter();
    insertExpandButtons();
    ...
    updateYearSpans(TWPHWLGD_data, TWPHWLGD_meta);

    const card_1_raw = TWPHWLGD_data
        .filter(row => row["Statistic"] == "Mean Hours Worked" &&
...
row["Working Pattern"] == "Full Time")
    ...
    const card_2_raw = TWPHWLGD_data
        .filter(row => row["Statistic"] == "Mean Hours Worked" &&
row["Year"] == latest_year &&
row["Local Government District"] == "Northern Ireland" &&
row["Working Pattern"] == "Part Time")
        .flatMap(row => ["Female", "All Persons"].map(column => row[column]))
        .reduce((sum, value) => sum + (Number(value) || 0), 0);

    // BuildR display value: 36.9
    ...
    const card_3_raw = TWPHWLGD_data
        .filter(row => row["Statistic"] == "Mean Hours Worked" &&
row["Year"] == latest_year &&
row["Local Government District"] == "Northern Ireland" &&
row["Working Pattern"] == "Full Time")
        .map(col => col["Female"])[0];

    // BuildR display value: 37.7
    const card_3_value = card_3_raw.toFixed(1);
    insertValue("card-3-value", card_3_value);


    // Content for card 4

    const card_4_raw = TWPHWLGD_data
        .filter(row => row["Statistic"] == "Mean Hours Worked" &&
row["Year"] == latest_year &&
row["Local Government District"] == "Northern Ireland" &&
row["Working Pattern"] == "Part Time")
    ...
    const card_5_raw = TWPHWLGD_data
        .filter(row => row["Statistic"] == "Mean Hours Worked" &&
row["Year"] == latest_year &&
row["Local Government District"] == "Northern Ireland" &&
row["Working Pattern"] == "Full Time")
    ...
    const card_6_raw = TWPHWLGD_data
        .filter(row => row["Statistic"] == "Mean Hours Worked" &&
row["Year"] == latest_year &&
row["Local Government District"] == "Northern Ireland" &&
row["Working Pattern"] == "Part Time")
    ...
    // BuildR matrix: TWPHWLGD

    // BuildR line chart config start
    let line_chart_1_years = TWPHWLGD_data
        .map(col => col["Year"]);
    ...
    const line_chart_1_lines = [
        TWPHWLGD_data
            .filter(row => line_chart_1_years.includes(row["Year"]) &&
                row["Statistic"] == "Mean Hours Worked" &&
                row["Local Government District"] == "Northern Ireland" &&
                row["Working Pattern"] == "Full Time")
    ...
    const line_chart_1_query = {
        "Statistic": "Mean Hours Worked",
        "Local Government District": "Northern Ireland",
        "Working Pattern": "Full Time",
        "Sex": ["All Persons", "Female", "Male"]
    };

    downloadButton(
        "chart-1-capture",
        "TWPHWLGD",
        dateFormat(TWPHWLGD_meta.updated),
        line_chart_1_query
    );
    ...
    const map_chart_2_data = TWPHWLGD_data
        .filter(row => row["Statistic"] == "Mean Hours Worked" &&
                       row["Year"] == latest_year &&
                       row["Working Pattern"] == "Full Time" &&
                       row["Local Government District"] != "Northern Ireland");
