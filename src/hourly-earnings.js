...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    initCookieConsent();
    await insertHead("Hourly earnings");
    insertHeader();
    insertNavButtons();
    insertFooter();
    insertExpandButtons();
...
row["Pay Rate"] == "Hourly (excluding overtime)" &&
row["Working Pattern"] == "Full-Time")
        .map(col => col["All Persons"])[0];

    // BuildR display value: 17.8
...
row["Working Pattern"] == "Full-Time")
    ...
    const card_3_value = card_3_raw.toFixed(2);
...
row["Working Pattern"] == "Part-Time")
    ...
    // BuildR display value: 13.56
    const card_4_value = card_4_raw.toFixed(2);
...
row["Working Pattern"] == "Full-Time")
    ...
    const card_5_value = card_5_raw.toFixed(2);
...
row["Working Pattern"] == "Part-Time")
    ...
    // BuildR display value: 12.92
    const card_6_value = card_6_raw.toFixed(2);
    ...
    const line_chart_1_labels = ["All employess", "Females", "Males"];
    ...
    const line_chart_1_query = {
        ...
        "Pay Rate": "Hourly (excluding overtime)",
    ...
    populateInfoBoxes(
        ...
        [
            // DEFINITIONS BOX
            `<p>Gross earnings relate to pay before deductions for PAYE, National Insurance, pension schemes, student loan repayments and voluntary deductions.</p>

<p>Full-time employee is defined as anyone aged 16 years or over that is directly paid from a business's payroll for carrying out more than 30 paid hours per week (or 25 or more for the teaching professions).</p>

<p>Part-time employee is defined as anyone aged 16 years or over working 30 or less paid hours a week (or less than 25 for the teaching professions).</p>

<p>Please note, the geographical data is based on the employees place of work.</p>`,
